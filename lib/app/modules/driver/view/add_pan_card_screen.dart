...
class AddPanCardScreen extends StatefulWidget {
  final String userId;
  const AddPanCardScreen({super.key, required this.userId});

  @override
  _AddPanCardScreenState createState() => _AddPanCardScreenState();
}

class _AddPanCardScreenState extends State<AddPanCardScreen> {
  File? imageFile;
  String? panNumber;
  ...
  Future<void> _uploadDocument(BuildContext context) async {
    if (panNumber == null || panNumber!.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please enter the PAN number')),
      );
      return;
    }
    if (imageFile == null) {
    ...
    try {
      ...
      if (docLink != null) {
        await UpdateDriverDocument().updatePan(
          widget.userId,
          panNumber!,
          docLink,
        );
